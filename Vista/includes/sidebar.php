...
| BASE_URL seguro
|--------------------------------------------------------------------------
| Evita pantalla blanca si BASE_URL no está definido en alguna vista.
*/
if (!defined('BASE_URL')) {
    define('BASE_URL', '');
}
...
/*
|--------------------------------------------------------------------------
| Cargar modelo de permisos de forma segura
|--------------------------------------------------------------------------
*/
if (!class_exists('MdPermisos')) {
    $rutaModeloPermisos = dirname(__DIR__, 2) . '/Modelo/MdPermisos.php';

    if (file_exists($rutaModeloPermisos)) {
        require_once $rutaModeloPermisos;
    }
}
...
if (!function_exists('tieneAcceso')) {
    function tieneAcceso($modulo)
    {
        ...
        */
        ...
        |--------------------------------------------------------------------------
        */
        if (class_exists('MdPermisos') && method_exists('MdPermisos', 'mdlTienePermisoModulo')) {
            return MdPermisos::mdlTienePermisoModulo($rol, $modulo);
        }

        /*
        |--------------------------------------------------------------------------
        | Respaldo por si existe tu función antigua check_access()
        |--------------------------------------------------------------------------
        */
...
$accesoUsuarios         = tieneAcceso('usuarios');
$accesoValidaciones     = tieneAcceso('validaciones');
$accesoLicencias        = tieneAcceso('licencias');
$accesoTeletrabajo      = tieneAcceso('teletrabajo');
...
|--------------------------------------------------------------------------
*/
if (!function_exists('itemSidebar')) {
...
<div class="md:hidden flex items-center justify-between bg-gradient-to-r from-[#1a0505] to-[#0d0202] p-4 border-b border-red-950/50">
    <div class="flex items-center">
        ...
        <span class="ml-3 text-lg font-bold text-white">
            RRHH<span class="text-red-400">Panel</span>
        </span>
    </div>

    <button id="btn-menu" class="text-white text-2xl leading-none">
        ☰
    </button>
</div>
...
<aside id="sidebar"
    ...
    <nav class="flex-1 overflow-y-auto py-6 custom-scrollbar">
        ...
        <!-- MI ESPACIO -->
        <?php if ($accesoPerfil || $accesoMisValidaciones || $accesoDocs): ?>
            ...
            <?php if ($accesoPerfil): ?>
                <?= itemSidebar(
                    BASE_URL . "/perfil",
                    '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M12 12c2.76 0 5-2.24 5-5s-2.24-5-5-5-5 2.24-5 5 2.24 5 5 5zm0 2c-3.31 0-8 1.67-8 5v1a2 2 0 002 2h12a2 2 0 002-2v-1c0-3.33-4.69-5-8-5z"/></svg>',
                    "Mi Perfil",
                    $menu_activo === 'perfil'
                ) ?>
            <?php endif; ?>
        ...
        <!-- ADMINISTRACIÓN -->
        <?php if ($accesoRRHH || $accesoValidaciones || $accesoContratos || $accesoLicencias || $accesoTeletrabajo): ?>
            ...
            <?php if ($accesoValidaciones): ?>
                <?= itemSidebar(
                    BASE_URL . "/rrhh/validaciones",
                    '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M9 12l2 2 4-4m1-7H8a2 2 0 00-2 2v14l6-3 6 3V5a2 2 0 00-2-2z"/></svg>',
                    "Validaciones",
            ...
            <?php if ($accesoLicencias): ?>
                <?= itemSidebar(
                    BASE_URL . "/rrhh/licencias",
                    '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M7 2h10a2 2 0 012 2v16l-7-3-7 3V4a2 2 0 012-2zm2 5v2h6V7H9zm0 4v2h6v-2H9z"/></svg>',
                    "Licencias",
                    $menu_activo === 'licencias'
                ) ?>
            <?php endif; ?>

            <?php if ($accesoTeletrabajo): ?>
                <?= itemSidebar(
                    BASE_URL . "/rrhh/teletrabajo",
                    '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M4 5a2 2 0 012-2h12a2 2 0 012 2v9a2 2 0 01-2 2h-5v2h3a1 1 0 110 2H8a1 1 0 110-2h3v-2H6a2 2 0 01-2-2V5zm2 0v9h12V5H6z"/></svg>',
                    "Teletrabajo",
        ...
        <!-- CONFIGURACIÓN -->
        <?php if ($accesoConfig || $accesoUsuarios): ?>
            <div class="text-[10px] font-bold text-white/30 uppercase tracking-widest px-6 mt-8 mb-3">
                Configuración
            </div>
    ...
    <a href="<?= BASE_URL ?>/logout"
        class="mx-3 mb-4 p-3 rounded-2xl bg-white/10 backdrop-blur-sm shadow-inner hover:bg-white/10 transition flex items-center gap-3 shrink-0">
        ...
        <div class="flex-1 overflow-hidden">
            <p class="text-sm font-bold truncate">
                <?= htmlspecialchars($_SESSION['nombre_completo'] ?? $_SESSION['username'] ?? 'Usuario') ?>
            </p>
